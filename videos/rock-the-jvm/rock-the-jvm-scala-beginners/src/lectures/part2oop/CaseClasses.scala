...
object CaseClasses extends App {
  ...
  // FUNDAMENTAL: nevertheless, all those "copied" instances have their own instance's reference

  // IMPORTANT: `copy` offers customization over the copied object via arguments:
  ...
  // Remember: when writing trailing `()`, you are really calling the `apply` method
  ...
  // -> IMPORTANT: in practice, this enables you to omit the keyword `new` when instantiating a case class
  ...
  // -> Enable case classes to be used in pattern matching (i.e., one of the most powerful Scala features)


  // There are also /case objects/
  // -> Same properties as case classes, except that they are objects (and therefore cannot have a companion object)
  case object UnitedKingdom {
    def name: String = "The UK of GB and NI"
  }

  // Example: making `Cons` and `Nil` a case class and a case object respectively enables `LinkedList` to be able to:
