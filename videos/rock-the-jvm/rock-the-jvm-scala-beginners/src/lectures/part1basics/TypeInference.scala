...
object TypeInference extends App {
  ...
  // The compiler is also able to infer the result type of a function by looking at the result type of the body:
  def successor(x: Int) = x + 1 // The result type of this function is `Int`

  // When explicitly writing type annotations, the compiler still does type inference to do a type check:
  ...
  // The second branch yields a product of an `Int` by the result of a function w/ a type to be determined next
